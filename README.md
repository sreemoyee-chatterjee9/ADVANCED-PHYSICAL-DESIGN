...
<pre>Utilization Factor = (Area Occupied by the Netlist)/(Total Area of the Core) 

Aspect Ratio = Height/Width</pre>
...
23. OVERLAP OF POLY LAYER AND N DIFFUSION RESULTS IN FORMATION OF **NMOS**

24. SEQUENCE TO FOLLOW TO EXTRACT .SPICE FILE WITH PARASITICS: extract all -> ext2spice cthresh 0 rthresh 0 -> ext2spice.

25. THE FALL_TRANSITION FOR THE GIVEN CIRCUIT IS: ~43ps

26. THE CELL_FALL DELAY FOR THE GIVEN CIRCUIT IS: ~28ps

27. WHAT DRC COMMAND CAN BE USED TO CHECK THE REASON OF VIOLATION UNDER A SELECTED BOX IN MAGIC LAYOUT WINDOW? - drc why

28. TWO IMPORTANT RESOURCES FOR SKYWATER 130NM PDK ARE: SkyWater webpage + google/skywater github page.

29. WHICH OF THE FOLLOWING CAN BE USED TO SEE THE IMPLANT LAYERS ON MAGIC LAYOUT WINDOW? - cif see [layers].

30. HOW TO REDIRECT ALL KEYSTROKES FROM LAYOUT WINDOW TO CONSOLE WINDOW? Pressing : or ; while pointer is in layout window.

31. ALIAS FOR PSUB IS : pwell.

32. WHAT COMMAND(S) FOLLOW A TECH FILE MODIFICATION TO REFLECT IN CURRENT LAYOUT WINDOW? - tech load sky130A.tech; drc check.
...
40. IF CLOCK PERIOD IS 10NS, WHAT IS THE CLOCK FREQUENCY? - 100MHz

41. IF CLOCK FREQUENCY IS 500MHZ, WHAT IS THE CLOCK PERIOD? - 2ns.

42. A BASIC FLIP-FLOP BUILT USING MUXES CAN HAVE `2 MUXes`

43. RISE EDGE TRIGGERED FLIP-FLOP LAUNCHES DATA AT `rise edge of clock signal`.

44. FOR CLOCK PERIOD OF 1NS, SETUP TIME OF 20PS AND SETUP UNCERTAINTY OF 90PS, WHAT IS THE COMINATIONAL DELAY (C) REQUIREMENT? - C < 0.89ns

45. IF T1 IS THE DELAY FROM CLOCK PORT TO LAUNCH CLOCK PIN, IF T2 IS THE DELAY FROM CLOCK PORT TO CAPTURE CLOCK PIN, WHAT IS THE VALUE OF SKEW ? - |t1 - t2|

46. IF AGGRESSOR SWITCHES FROM LOGIC 1 TO 0, AND IF VICTIM IS STABLE AT LOGIC 1, HOW WILL THE AGGRESSOR IMPACT VICTIM? - There will be a glitch on victim net making it to temporarily switch from logic 1 to logic 0 and back to logic 1.

47. IF AGGRESSOR SWITCHES FROM LOGIC 0 TO 1, AND IF VICTIM SWITCHES FROM LOGIC 1 TO LOGIC 0, HOW WILL THE AGGRESSOR IMPACT VICTIM? - While switching from logic 1 to logic 0, victim will momentarily try to switch to logic 1 and drops to logic 0 creating a delta delay bump on victim waveform.

48. IF CLOCK PERIOD IS 1NS, SETUP TIME IS 10PS, SETUP UNCERTAINTY IS 90PS, LAUNCH CLOCK NETWORK DELAY IS 50PS, CAPTURE CLOCK NETWORK DELAY IS 50PS, WHAT IS THE COMBINATIONAL DELAY (C) REQUIREMENT? C < 0.9ns.

49. WHAT DOES THE MARKED TCL COMMAND DO TO THE LIST DEFINED AS BELOW?
    
    \NLIST -> $::ENV(CTS_CLK_BUFFER_LIST) = {CLKBUF_1 CLKBUF_2 CLKBUF_4 CLKBUF_8}\NCOMMAND -> LREPLACE ::ENV(CTS_CLK_BUFFER_LIST) 1 1\N
    
    - Does no modification to the list ::env(CTS_CLK_BUFFER_LIST)
    
50. FOR THE IMPROVED NETLIST*, WHATiS THE HOLD SLACK WITH DEFAULT BUFFER LIST?

    \N(* IMPROVED NETLIST -> NETLIST WITH POST SYNTHESIS SETUP SLACK = -0.12)\N
    
    - 0.4247
    
51. FOR THE IMPROVED NETLIST*, WHATS THE HOLD CLOCK SKEW WITH DEFAULT BUFFER LIST?

    \N(* IMPROVED NETLIST -> NETLIST WITH POST SYNTHESIS SETUP SLACK = -0.12 PRESENT IN ./VSDSTDCELLDESIGN/EXTRAS)\N
    
    - 0.3
    
52. FOR THE IMPROVED NETLIST*, WHATS THE SETUP SLACK WITH DEFAULT BUFFER LIST?

    \N(* IMPROVED NETLIST -> NETLIST WITH POST SYNTHESIS SETUP SLACK = -0.12 ./VSDSTDCELLDESIGN/EXTRAS)\N
    
    - 4.449
    
53. FOR THE IMPROVED NETLIST*, WHATS THE SETUP CLOCK SKEW WITH DEFAULT BUFFER LIST?

    \N(* IMPROVED NETLIST -> NETLIST WITH POST SYNTHESIS SETUP SLACK = -0.12 ./VSDSTDCELLDESIGN/EXTRAS)\N
    
    - 0.3
   
54. MAZE ALGORITHM STARTS BY CREATING A `Routing Grid`.

55. TO CONNECT 2 NODES, WHICH ROUTING STYLE WILL BE PREFERRED? - Routing with close to 1 bend

56. WIRE WIDTH DRC RULE IS MAINLY DUE TO WHICH MANUFATURING STEP IN 16-MASK CMOS PROCESS? - lithography.

57. WHAT DOES DRC RULE CALLED "WIRE-PITCH" MEAN? - Its centre to centre distance between 2 wires.

58. EASIEST AND CHEAPEST WAY TO FIX SIGNAL SHORTS IS `change metal layers`.

59. VIA WIDTH AND VIA SPACING DRC RULES ARE RELATED TO WHICH MANUFACTURING STEP? - lithography.

60. WHAT IS THE FULL-FORM OF SPEF? - Standard Parasitics Exchange Format.

61. WHAT DOES *L AND *C REPRESENTS IN SPEF FILE? - load and co-ordinate.

62. WHAT DOES *D AND *S REPRESENTS IN SPEF FILE? - driver and waveform shape.

63. IN BELOW SPEF SYNTAX, WHAT DOES I AND *L REPRESENTS? *PORTS, DIN2 I *L 0.05 - Input Port and Load.

64. WHERE DOES THE PDN TAKES POWER (AND GROUND) FROM? - From power and ground pads.

65. DETAILED ROUTING FOLLOWS THE GUIDES OBTAINED FROM THE FAST ROUTE TO ENSURE CONNECTIVITY. - True Statement.
